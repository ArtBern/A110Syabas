...
void tangox_machine_restart(char *command)
{
	...
	unsigned long base_addr, tmp;
	int loop, i;

    local_irq_disable();

	/* Turn on green LED to indicate restart */
	gbus_write_uint32(pGBus, REG_BASE_system_block + SYS_gpio_dir, (0x00010000 | 1) << 14);
	gbus_write_uint32(pGBus, REG_BASE_system_block + SYS_gpio_data, (0x00010000 | 1) << 14);

	gbus_write_uint32(pGBus, REG_BASE_system_block + SYS_gpio_dir, (0x00010000 | 1) << 15);
	gbus_write_uint32(pGBus, REG_BASE_system_block + SYS_gpio_data, (0x00010000 | 1) << 15);
	...
	gbus_writel(REG_BASE_demux_engine_0 + G2L_RESET_CONTROL, 3);
	gbus_writel(REG_BASE_demux_engine_1 + G2L_RESET_CONTROL, 3);
	udelay(1);
	gbus_writel(REG_BASE_demux_engine_0 + G2L_RESET_CONTROL, 2);
	gbus_writel(REG_BASE_demux_engine_1 + G2L_RESET_CONTROL, 2);
#else
	/* Resetting Transport demux block */
...
void tangox_machine_power_off(void)
{
	...
	/* Turn off red LED */
	gbus_write_uint32(pGBus, REG_BASE_system_block + SYS_gpio_dir, (0x00010000 | 1) << 15);
	gbus_write_uint32(pGBus, REG_BASE_system_block + SYS_gpio_data, (0x00010000 | 0) << 15);

	/* Turn off green LED */
	gbus_write_uint32(pGBus, REG_BASE_system_block + SYS_gpio_dir, (0x00010000 | 1) << 14);
	gbus_write_uint32(pGBus, REG_BASE_system_block + SYS_gpio_data, (0x00010000 | 0) << 14);

	/* Turn off FIP display */
	while(gbus_read_uint32(pGBus, REG_BASE_system_block + 0x558) & 0x200);
	gbus_write_uint32(pGBus, REG_BASE_system_block + 0x540, 0x80);
	while(gbus_read_uint32(pGBus, REG_BASE_system_block + 0x558) & 0x200);

	/* Turn off Ethernet PHY */
	for (phy = 1; phy < 32; phy++) 
	{
		...
		/* Read MII_PHYSID1 */
		while (gbus_read_uint32(pGBus, REG_BASE_host_interface + 0x6014) & 1);
		gbus_write_uint32(pGBus, REG_BASE_host_interface + 0x6014, (phy << 11) | (0x02 < 6));
		udelay(1);
		while (gbus_read_uint32(pGBus, REG_BASE_host_interface + 0x6014) & 1);
		val = gbus_read_uint32(pGBus, REG_BASE_system_block + 0x6018);
		id = (val << 16);

		/* Read MII_PHYSID2 */
		while (gbus_read_uint32(pGBus, REG_BASE_host_interface + 0x6014) & 1);
		gbus_write_uint32(pGBus, REG_BASE_host_interface + 0x6014, (phy << 11) | (0x03 < 6));
		udelay(1);
		while (gbus_read_uint32(pGBus, REG_BASE_host_interface + 0x6014) & 1);
		val = gbus_read_uint32(pGBus, REG_BASE_host_interface + 0x6018);
		id |= val;

		if (id != 0xffffffff && id != 0x00000000)
			break;
	}
	gbus_write_uint32(pGBus, REG_BASE_host_interface + 0x6018, 0x0800);
	gbus_write_uint32(pGBus, REG_BASE_host_interface + 0x6014, (phy << 11) | (0 << 6) | (1 << 1));
	udelay(1);
	while (gbus_read_uint32(pGBus, REG_BASE_host_interface + 0x6014) & 1);

	/* Turn off Ethernet DMA */
	gbus_write_uint32(pGBus, REG_BASE_host_interface + 0x701C, 0);
	gbus_write_uint32(pGBus, REG_BASE_host_interface + 0x7018, 0);
