		    ...
		    unsigned int len)
{
	...
	extern void flush_dcache_range(void __user *userbuf, unsigned int len);
	unsigned long paddr;
	...
	if (!(uaddr & dma_alignment) && !(len & dma_alignment)) {
#ifdef CONFIG_SD_DIRECT_DMA
		if (((reading && access_ok(VERIFY_WRITE, (void __user *)ubuf, len)) || 
		    (!reading && access_ok(VERIFY_READ, (void __user *)ubuf, len))) && 
		    (len >= 131072) && is_contiguous_memory(ubuf, len, &paddr) && 
		    (disk->major == IDE0_MAJOR)) { /* Only with IDE interface */
