...
#include <asm/tango2/fip-user.h>
#include <asm/tango2/fip-map.h>
#endif
#endif /* 1 */
...
#if 0 /* BLTeo, already defined these in other file */
#if defined(CONFIG_TANGOX_FIP_REF1)
...
#if 1 /* BLTeo, changed to suit our front panel */
			byte1 = 3 * (position - 1);
			byte2 = 3 * (position - 1) + 1 ;
#else
			byte1 = 24 - (3 * position);
			byte2 = 25 - (3 * position);
#endif /* 1 */
...
#if 0 /* BLTeo, our front panel has no colons */
...
#if 1 /* BLTeo, show text */
	fip_write_text(0, "LOADING", FIP_LEFT);
#endif /* 1 */
