...
struct ir_private {
	struct key *buffer;		/* Circular buffer */
...
static void ir_produce(struct ir_private *priv, unsigned long status)
{
	...
	if ((status & 0x80000000) != 0) {	// RC6 Data in IRQ
		...
		priv->buffer[priv->p_idx].data = dx[0];
		priv->p_idx = (priv->p_idx + 1) % buffer_size;
		priv->buffer[priv->p_idx].data = dx[1];
		priv->p_idx = (priv->p_idx + 1) % buffer_size;
		priv->buffer[priv->p_idx].data = dx[2];
		priv->p_idx = (priv->p_idx + 1) % buffer_size;
		priv->buffer[priv->p_idx].data = dx[3];
		priv->p_idx = (priv->p_idx + 1) % buffer_size;
		priv->buffer[priv->p_idx].data = dx[4];
		priv->p_idx = (priv->p_idx + 1) % buffer_size;
	...
	if (status & 0x00000001) {	// RC5 IRQ
		key.data = gbus_read_uint32(pGBus, IR_RC5_DECODER_DATA);
		gbus_write_uint32(pGBus, IR_RC5_DECODER_DATA, 0);
		if ((key.data & 0x80000000) != 0)  /* Invalid RC5 decoder data */
			goto out;
	} else if (status & 0x00000002) {	// NEC IRQ
		key.data = gbus_read_uint32(pGBus, IR_NEC_CAPTURE_DATA);
...
#endif
#endif
		key.data = save_key; /* Valid repeated key */
		key.repeated = 1;
		repeat_key = 1;
	} else if (time_after(priv->last_jiffies + wait_period, jiffies) &&
			(key.data == old_key)) {
	...
	memcpy(&priv->buffer[priv->p_idx], &key, sizeof(struct key));
	priv->p_idx = (priv->p_idx + 1) % buffer_size;
...
/* Reading from driver's buffer, note that it can return read size
   less than specified */
static int ir_consume(void *dev_id, struct key *buf, int count)
{
	struct ir_private *priv = (struct ir_private *)dev_id;
	int cnt;
	unsigned long flags;
	...
	for (cnt = 0; (priv->c_idx != priv->p_idx) && (cnt < count); cnt++) {
		//buf->data = priv->buffer[priv->c_idx].data;
		//buf->repeated = priv->buffer[priv->c_idx].repeated;
		//*buf = priv->buffer[priv->c_idx];
		memcpy(buf, &priv->buffer[priv->c_idx], sizeof(struct key));
		priv->c_idx = (priv->c_idx + 1) % buffer_size;
		buf++;
	}
...
static int ir_read(struct file *fptr, char *bufptr, size_t size, loff_t *fp)
{
	...
	size / sizeof(struct key)) * sizeof(struct key);
...
int __init ir_init_module(void)
{
	...
	if ((ir_priv.buffer = kmalloc(buffer_size * sizeof(struct key), GFP_KERNEL)) == NULL) {
	...
	printk(KERN_INFO "SMP863x %s (%d:0): driver loaded (wait_period = %dms, "
		"buffer_size = %d)\n", ir_devname, major, wait_period, buffer_size);
	return(0);
}
