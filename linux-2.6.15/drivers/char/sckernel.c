...
#define SCARD2_OFF_PIN          0  /* SCARD_CTL[0] */
#define SCARD2_5V_PIN           1  /* SCARD_CTL[1] */
#define SCARD2_CMD_PIN          2  /* SCARD_CTL[2] */
...
#ifdef CONFIG_TANGO3
	{ (REG_BASE_cpu_block + 0xc380), SCARD2_5V_PIN, SCARD2_CMD_PIN, SCARD2_OFF_PIN, IRQ_CONTROLLER_IRQ_BASE + LOG2_CPU_SMARTCARD2_INT },
#endif
};
...
static int xenv_scard_5v_pin, xenv_scard_off_pin, xenv_scard_cmd_pin;
...
#elif defined(CONFIG_TANGO3)
	unsigned int v = ((1 << scard_priv[idx].scard_cmd_pin) | (1 << scard_priv[idx].scard_5v_pin)) << 4;
	v |= 0x7000; /* With OE mask set */
	gbus_write_uint32(pGBus, SCARD_GPIO_MODE_REG(scard_priv[idx].base_addr), 0x7f00); /* Set smard card mode */
...
//		gbus_write_uint32(pGBus, SCARD_SOFT_OUT_REG(priv->base_addr), gbus_read_uint32(pGBus, SCARD_SOFT_OUT_REG(priv->base_addr)) | 0x620);
		...
		if (atomic_read(&(priv->answer2reset)) != 0) { 
			...
			DBG_PRINT("\n"); 
#endif
			if ((ret = parse_ans2reset(priv, 1, priv->ans_string, priv->ans_idx)) < 0) {
...
//		gbus_write_uint32(pGBus, SCARD_SOFT_OUT_REG(priv->base_addr), gbus_read_uint32(pGBus, SCARD_SOFT_OUT_REG(priv->base_addr)) | 0x620);
		...
		if (atomic_read(&(priv->answer2reset)) != 0) { 
			...
			DBG_PRINT("\n"); 
#endif
			if ((ret = parse_ans2reset(priv, 0, priv->ans_string, priv->ans_idx)) < 0) {
				DBG_PRINT("Ans2reset: string error\n");
				scard_reset(priv, 1); /* cold reset */
			} else if (ret > 0) {
				/* Perform warm reset */
...
static int scard_read(struct file *fptr, char *buffer, size_t count, loff_t *fp)
{
	...
	if (priv->b_mode != 0) {
		...
		while (priv->rd_c_idx == priv->rd_p_idx) {
			spin_unlock_irqrestore(&priv->lock, flags);
			interruptible_sleep_on(&(priv->scard_rd_wq));
			if (atomic_read(&(priv->card_existed)) == 0) 
				return(-EIO);
			spin_lock_irqsave(&priv->lock, flags);
		}
...
int __init scard_init_module(void)
{
	...
	extern int tangox_scard_enabled(void);
	...
	tangox_get_scard_info(&xenv_scard_5v_pin, &xenv_scard_cmd_pin, &xenv_scard_off_pin);

	/* override the default from XENV settings */
	scard_priv[0].scard_5v_pin = xenv_scard_5v_pin;
	scard_priv[0].scard_cmd_pin = xenv_scard_cmd_pin;
	scard_priv[0].scard_off_pin = xenv_scard_off_pin;

#ifdef CONFIG_TANGO3
	/* TODO: how to get information for 2nd scard controller, XENV as well? */
#endif

	/* Initialize private data structure */
	for (i = 0; i < NUM_SCARD; i++) {
		memset(&scard_priv[i], 0, sizeof(struct scard_private)); 
