...
static int __init init_physmap(void)
{
	...
	for (cs = 0; cs < XENV_MAX_FLASH; cs++) {
		...
		if (mymtds[cs] && (mymtds[cs]->size != f_sizes[cs])) { 
			...
			printk("CS%d: mymtds[cs]->size %06x f_sizes[cs] %06x \n", cs, mymtds[cs]->size, f_sizes[cs]);
		...
		if (mymtds[cs]) {
			...
			printk("add_mtd_device first cs %d es %06x sz %06x esregion %d %06x %06x\r\n", cs, mymtds[cs]->erasesize, mymtds[cs]->size, mymtds[cs]->numeraseregions,
			       mymtds[cs]->eraseregions[0].offset, mymtds[cs]->eraseregions[0].erasesize);
			if (mymtds[cs]->numeraseregions > 1)
				printk("esregion 1:  %06x %06x\r\n", mymtds[cs]->eraseregions[1].offset, mymtds[cs]->eraseregions[1].erasesize);
...
#if 1
						/*printk("mymtds[0]->erasesize %06x %06x %06x\r\n",
							mymtds[0]->erasesize, mymtds[1]->erasesize, mymtds[2]->erasesize);*/
						//setting size should >= 0x10000;//64k
						if(mymtds[cs]->erasesize > 0x10000)
							part_ptr->size = mymtds[cs]->erasesize;
						else
							part_ptr->size = 0x10000;
						part_ptr->offset = physmap_maps[cs].size -part_ptr->size;
#else //obsolete as not align to the erase block
						part_ptr->size = 0x4000;//16k
						part_ptr->offset = physmap_maps[cs].size - 0x4000;//0x0FFC000;
#endif
						//szs[0] is siz of flash, size of flash -16k is the loc of setting

						//sprintf(part_ptr->name, "env16k");//maximum 16 character as above
						sprintf(part_ptr->name, "Setting %06x", (unsigned int) part_ptr->offset) ;
					}
					else
					if (p+1 == 2)
					{

						//sprintf(part_ptr->name, "All15.00M!");
						part_ptr->size = physmap_maps[cs].size - 0x100000;//total flash size - yamon - zboot - setting
						part_ptr->offset = 0x80000;//
						sprintf(part_ptr->name, "Kernel %d", part_ptr->size);
					}
					else if (p+1 == 3)
					{
						//sprintf(part_ptr->name, "App %d", part_ptr->size);
					#ifdef MAXKERNELSIZE
						part_ptr->size = physmap_maps[cs].size - MAXKERNELSIZE - 0x100000;
						part_ptr->offset = 0x80000 + MAXKERNELSIZE;
					#else
						part_ptr->size = physmap_maps[cs].size - 0x500000;
						part_ptr->offset = 0x80000 + 0x500000;
					#endif
						sprintf(part_ptr->name, "App %d", part_ptr->size);
					}
					else if (p+1 == 4)
					{
						sprintf(part_ptr->name, "Yamon");
						part_ptr->size = 0x40000;
						part_ptr->offset = 0x40000;

					}
					else if (p+1 == 5)
					{
						sprintf(part_ptr->name, "Zboot");
						part_ptr->size = 0x40000;
						part_ptr->offset = 0x0;

					}
					else
#endif /* def SYABAS_PORT */
					if (part_ptr->name != NULL) 
						sprintf(part_ptr->name, "CS%d-Part%d", cs, p + 1);

					printk("MTD CS%d-Part%d (offset:0x%x, size:0x%x) \n",
						cs, p + 1, offsets[p], szs[p] & 0x7fffffff);
					pcnt++;
					part_ptr++;
				}
	...
	if (mymtd) {
		...
		if (mtd_parts_nb > 0)
		{
			printk("MTD b4 add_mtd_partitions, %d\r\n", mtd_parts_nb);
