...
#if 1 /* BLTeo */
/*
 * setting mac address from user space 
 */
static int enet_set_mac_address_usr(struct net_device *dev, void *addr)
{
	struct sockaddr *sockaddr = (struct sockaddr *) addr;
	return enet_set_mac_address(dev, &(sockaddr->sa_data));
}
#endif /* 1 */
 ...
 */
extern int tangox_ethernet_getmac(unsigned char *);
...
#if 1 /* BLTeo */
	dev->set_mac_address = enet_set_mac_address_usr;
#else
...
#if 1 /* BLTeo */
	/* [20070530] BLTeo, set the mac address to 0 first, reset in user space */
	memset(dev->dev_addr, 0, 6);
#else
	tangox_ethernet_getmac(dev->dev_addr);
#endif /* 1 */
	enet_set_mac_address(dev, dev->dev_addr);
