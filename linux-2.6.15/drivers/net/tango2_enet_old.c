...
extern int tangox_ethernet_getmac(unsigned char *mac);
#endif
...
void setup_phy (struct mii_if_info *mii_if)
{
	...
	else if (phy_caps & BMSR_100FULL)
		ad_caps |= ADVERTISE_100FULL;
	else if (phy_caps & BMSR_100HALF)
		ad_caps |= ADVERTISE_100HALF;
	else if (phy_caps & BMSR_10FULL)
		ad_caps |= ADVERTISE_10FULL;
	else if (phy_caps & BMSR_10HALF)
...
static int em86xx_get_macaddr(unsigned char *addr)
{
	...
	if (get_mac_address(&lo, &hi) == 0) {
		...
		*addr   = (lo & 0x000000ff);
	} else if (tangox_ethernet_getmac(addr) == 0) {
