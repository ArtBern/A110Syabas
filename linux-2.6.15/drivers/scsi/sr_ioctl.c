...
int sr_drive_status(struct cdrom_device_info *cdi, int slot)
{
	...
	if (0 == sr_do_ioctl(cdi->handle, &cgc) || sense.sense_key == UNIT_ATTENTION)
		return CDS_DISC_OK;
	...
	if (0 == sr_do_ioctl(cdi->handle, &cgc))
		return CDS_DISC_OK;
	...
	if (sense.sense_key == NOT_READY) {
		if (sense.asc == 0x3a) {
			if (sense.ascq == 1)
			{
				//printfk("CDS_NO_DISC at ide-cd ascq=%d\n", sense.ascq);
				return CDS_NO_DISC;
			}
			else if (sense.ascq == 0 || sense.ascq == 2)
			{
				//printfk("CDS_TRAY_OPEN at ide-cd ascq=%d\n", sense.ascq);
				return CDS_TRAY_OPEN;
			}
		}
	}
