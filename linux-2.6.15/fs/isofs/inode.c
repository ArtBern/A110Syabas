		     ...
		     struct buffer_head **bh, unsigned long nblocks)
{
	...
	while ( nblocks ) {
		...
		if (b_off > ((inode->i_size + PAGE_CACHE_SIZE - 1) >> ISOFS_BUFFER_BITS(inode))) {
			printk(KERN_ERR "isofs_get_blocks: block >= EOF (%ld, %ld)\n",
...
static int
isofs_direct_io_get_block(struct inode *inode, sector_t iblock,
		...
		int create)
{
	...
	if((((long) iblock << ISOFS_BUFFER_BITS(inode)) + PAGE_CACHE_SIZE - 1) > inode->i_size) 
...
static ssize_t isofs_direct_IO(int rw, struct kiocb *iocb,
			...
			unsigned long nr_segs)
{
	...
	return blockdev_direct_IO(rw, iocb, inode, inode->i_sb->s_bdev, iov,
				 offset, nr_segs,
				 isofs_direct_io_get_block, NULL);
}
